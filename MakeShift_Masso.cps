/**
  ...
  $Revision: 43976 30f784118b78098bfab508c19076f5ea1124289f $
  $Date: 2022-10-11 09:31:13 $
...
function onOpen() {
  ...
  if (getProperty("fourthAxis") != "none" && getProperty("fifthAxis")) {
  ...
  if (getProperty("fifthAxis")) { // note: setup your machine here
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-110, 110], preference:-1});
    var cAxis = createAxis({coordinate:1, table:true, axis:[0, 0, 1], cyclic:true});
    machineConfiguration = new MachineConfiguration(aAxis, cAxis);

    setMachineConfiguration(machineConfiguration);
    optimizeMachineAngles2(2); // TCP mode
  }
  ...
  var tmpNum = parseInt(programName, 10);
  if (isNaN(tmpNum)) {
    lastSubprogram = 1000;
  } else {
    lastSubprogram = tmpNum;
  }
...
function defineWorkPlane(_section, _setWorkPlane) {
  ...
  if (machineConfiguration.isMultiAxisConfiguration()) { // use 5-axis indexing for multi-axis mode
    ...
    if (_section.isMultiAxis()) {
      ...
      if (_setWorkPlane) {
        if (!(getProperty("fifthAxis") || getProperty("fourthAxis") != "none")) {
          error(localize("This post configuration has not been customized for simultaneous toolpath."));
        }
        if (!retracted) {
          writeRetract(Z);
        }
...
function subprogramStart(_initialPosition, _abc, _incremental) {
  var path = FileSystem.getCombinedPath(FileSystem.getFolderPath(getOutputPath()), oFormat.format(currentSubprogram) + "." + extension);
  redirectToFile(path);
  var comment = "";
  ...
  writeln(
    formatComment(oFormat.format(currentSubprogram)) +
...
function onLinear5D(_x, _y, _z, _a, _b, _c, feed) {
  ...
  if (x || y || z || a || b || c) {
    ...
    if (getNextRecord().isMotion()) { // try not to output feed without motion
      ...
      writeBlock(gFeedModeModal.format(f.fmode));
      writeBlock(gMotionModal.format(1), f.frn);
    }
  }
}
